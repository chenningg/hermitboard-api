...
type AssetQuery struct {
	...
	withDailyAssetPrice  *DailyAssetPriceQuery
	withFKs              bool
...
// QueryDailyAssetPrice chains the current query on the "daily_asset_price" edge.
func (aq *AssetQuery) QueryDailyAssetPrice() *DailyAssetPriceQuery {
...
func (aq *AssetQuery) FirstID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (aq *AssetQuery) FirstIDX(ctx context.Context) ulid.ULID {
...
func (aq *AssetQuery) OnlyID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (aq *AssetQuery) OnlyIDX(ctx context.Context) ulid.ULID {
...
func (aq *AssetQuery) IDs(ctx context.Context) ([]ulid.ULID, error) {
	var ids []ulid.ULID
...
func (aq *AssetQuery) IDsX(ctx context.Context) []ulid.ULID {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AssetQuery) Clone() *AssetQuery {
	...
	return &AssetQuery{
		...
		withDailyAssetPrice:  aq.withDailyAssetPrice.Clone(),
...
// WithDailyAssetPrice tells the query-builder to eager-load the nodes that are connected to
// the "daily_asset_price" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AssetQuery) WithDailyAssetPrice(opts ...func(*DailyAssetPriceQuery)) *AssetQuery {
	query := &DailyAssetPriceQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withDailyAssetPrice = query
	return aq
}
...
func (aq *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := aq.withDailyAssetPrice; query != nil {
		if err := aq.loadDailyAssetPrice(ctx, query, nodes,
			func(n *Asset) { n.Edges.DailyAssetPrice = []*DailyAssetPrice{} },
			func(n *Asset, e *DailyAssetPrice) { n.Edges.DailyAssetPrice = append(n.Edges.DailyAssetPrice, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AssetQuery) loadAssetClass(ctx context.Context, query *AssetClassQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *AssetClass)) error {
	...
	nodeids := make(map[ulid.ULID][]*Asset)
	for i := range nodes {
...
func (aq *AssetQuery) loadTransactionBase(ctx context.Context, query *TransactionQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Transaction)) error {
	...
	byID := make(map[ulid.ULID]*Asset)
	nids := make(map[ulid.ULID]map[*Asset]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(asset.TransactionBaseTable)
		s.Join(joinT).On(s.C(transaction.FieldID), joinT.C(asset.TransactionBasePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(asset.TransactionBasePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(asset.TransactionBasePrimaryKey[1]))
	...
	neighbors, err := query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
		...
		spec.Assign = func(columns []string, values []interface{}) error {
			...
			if nids[inValue] == nil {
				nids[inValue] = map[*Asset]struct{}{byID[outValue]: struct{}{}}
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "transaction_base" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (aq *AssetQuery) loadTransactionQuote(ctx context.Context, query *TransactionQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Transaction)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(asset.TransactionQuoteTable)
		s.Join(joinT).On(s.C(transaction.FieldID), joinT.C(asset.TransactionQuotePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(asset.TransactionQuotePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(asset.TransactionQuotePrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "transaction_quote" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (aq *AssetQuery) loadDailyAssetPrice(ctx context.Context, query *DailyAssetPriceQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *DailyAssetPrice)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[ulid.ULID]*Asset)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.DailyAssetPrice(func(s *sql.Selector) {
		s.Where(sql.InValues(asset.DailyAssetPriceColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.asset_daily_asset_price
		if fk == nil {
			return fmt.Errorf(`foreign-key "asset_daily_asset_price" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "asset_daily_asset_price" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
