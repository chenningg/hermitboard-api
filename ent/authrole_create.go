...
func (arc *AuthRoleCreate) check() error {
	...
	if _, ok := arc.mutation.Role(); !ok {
		return &ValidationError{Name: "role", err: errors.New(`ent: missing required field "AuthRole.role"`)}
	}
	if v, ok := arc.mutation.Role(); ok {
		if err := authrole.RoleValidator(v); err != nil {
			return &ValidationError{Name: "role", err: fmt.Errorf(`ent: validator failed for field "AuthRole.role": %w`, err)}
		}
	}
