...
func (cc *ConnectionCreate) AddPortfolioIDs(ids ...pulid.PULID) *ConnectionCreate {
	cc.mutation.AddPortfolioIDs(ids...)
	return cc
}
...
func (cc *ConnectionCreate) check() error {
	...
	if _, ok := cc.mutation.AccessToken(); !ok {
		return &ValidationError{Name: "access_token", err: errors.New(`ent: missing required field "Connection.access_token"`)}
	}
	if v, ok := cc.mutation.AccessToken(); ok {
...
func (cc *ConnectionCreate) createSpec() (*Connection, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.AccessToken(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
	...
	if nodes := cc.mutation.PortfoliosIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
