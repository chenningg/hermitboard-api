...
type Cryptocurrency struct {
	...
	asset_cryptocurrency *ulid.ULID
}
...
func (*Cryptocurrency) scanValues(columns []string) ([]interface{}, error) {
	values := make([]interface{}, len(columns))
	for i := range columns {
		switch columns[i] {
		case cryptocurrency.FieldSymbol, cryptocurrency.FieldIcon, cryptocurrency.FieldName:
			values[i] = new(sql.NullString)
		case cryptocurrency.FieldCreatedAt, cryptocurrency.FieldUpdatedAt:
...
func (c *Cryptocurrency) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case cryptocurrency.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				c.asset_cryptocurrency = new(ulid.ULID)
				*c.asset_cryptocurrency = *value.S.(*ulid.ULID)
			}
		}
	}
	return nil
