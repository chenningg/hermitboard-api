...
type DailyAssetPrice struct {
	...
	asset_daily_asset_price *ulid.ULID
}
...
func (*DailyAssetPrice) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case dailyassetprice.FieldCreatedAt, dailyassetprice.FieldUpdatedAt, dailyassetprice.FieldTime:
			values[i] = new(sql.NullTime)
		case dailyassetprice.FieldID:
			values[i] = new(ulid.ULID)
		case dailyassetprice.ForeignKeys[0]: // asset_daily_asset_price
...
func (dap *DailyAssetPrice) assignValues(columns []string, values []interface{}) error {
	...
	for i := range columns {
		...
		case dailyassetprice.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field asset_daily_asset_price", values[i])
			} else if value.Valid {
				dap.asset_daily_asset_price = new(ulid.ULID)
				*dap.asset_daily_asset_price = *value.S.(*ulid.ULID)
			}
		}
	}
	return nil
