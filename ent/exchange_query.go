...
func (eq *ExchangeQuery) FirstID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (eq *ExchangeQuery) FirstIDX(ctx context.Context) ulid.ULID {
...
func (eq *ExchangeQuery) OnlyID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (eq *ExchangeQuery) OnlyIDX(ctx context.Context) ulid.ULID {
...
func (eq *ExchangeQuery) IDs(ctx context.Context) ([]ulid.ULID, error) {
	var ids []ulid.ULID
...
func (eq *ExchangeQuery) IDsX(ctx context.Context) []ulid.ULID {
	ids, err := eq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (eq *ExchangeQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Exchange, init func(*Exchange), assign func(*Exchange, *Transaction)) error {
	...
	nodeids := make(map[ulid.ULID]*Exchange)
	for i := range nodes {
	...
	for _, n := range neighbors {
		fk := n.transaction_exchange
		if fk == nil {
			return fmt.Errorf(`foreign-key "transaction_exchange" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "transaction_exchange" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
