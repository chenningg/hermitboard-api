...
type Transaction struct {
	...
	portfolio_transactions       *ulid.ULID
	transaction_transaction_type *ulid.ULID
	transaction_exchange         *ulid.ULID
}
...
func (*Transaction) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case transaction.FieldCreatedAt, transaction.FieldUpdatedAt, transaction.FieldTime:
		...
		case transaction.ForeignKeys[1]: // transaction_transaction_type
			values[i] = &sql.NullScanner{S: new(ulid.ULID)}
		case transaction.ForeignKeys[2]: // transaction_exchange
			values[i] = &sql.NullScanner{S: new(ulid.ULID)}
		default:
	...
	for i := range columns {
		...
		case transaction.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				t.portfolio_transactions = new(ulid.ULID)
				*t.portfolio_transactions = *value.S.(*ulid.ULID)
			}
		case transaction.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field transaction_transaction_type", values[i])
			} else if value.Valid {
				t.transaction_transaction_type = new(ulid.ULID)
				*t.transaction_transaction_type = *value.S.(*ulid.ULID)
			}
		case transaction.ForeignKeys[2]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field transaction_exchange", values[i])
			} else if value.Valid {
				t.transaction_exchange = new(ulid.ULID)
				*t.transaction_exchange = *value.S.(*ulid.ULID)
			}
		}
	}
	return nil
